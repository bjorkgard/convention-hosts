...
export default function FloorRow({
    ...
    return (
        <Collapsible
            ...
            className="rounded-xl border border-border transition-colors duration-200 hover:border-primary/20"
        >
            ...
            <CollapsibleContent>
                {sections.length === 0 ? (
                    <p className="px-4 pb-3 text-sm text-muted-foreground">
                        {t('floor.row.no_sections')}
                    </p>
                ) : (
                    <ul className="border-t">
                        {sections.map((section) => {
                            const showEditSection =
                                canEditSection(userRole) && !!onEditSection;
                            const showDeleteSection =
                                canDeleteSection(userRole) && !!onDeleteSection;
                                            ...
                                            <div className="flex shrink-0 items-center gap-0.5 pr-2 sm:pr-3">
                                                {showEditSection && (
                                                    <Tooltip>
                                                        ...
                                                        <TooltipContent>
                                                            {t(
                                                                'floor.row.edit_section_tooltip',
                                                            )}
                                                        </TooltipContent>
                                                    </Tooltip>
                                                )}
                                                {showDeleteSection && (
                                                    <Tooltip>
                                                        ...
                                                        <TooltipContent>
                                                            {t(
                                                                'floor.row.delete_section_tooltip',
                                                            )}
                                                        </TooltipContent>
                                                    </Tooltip>
                                                )}
