...
Convention
  ...
  │       ├─ Occupancy tracking
  │       └─ Attendance reports
  ├─ Users (with roles: Owner, Administrator)
  ├─ URL tokens (section_url_token for anonymous access)
  └─ Attendance Periods
```
...
- Have a locale preference for internationalization
...
- Can be updated by users with convention access (Owner or Administrator)
- Can be deleted by Owner role only
- Have an auto-generated `section_url_token` (64-char random string) for anonymous URL access
- Token can be regenerated by authenticated users to invalidate old URLs

### Floors
- Physical levels within a convention venue
- Belong to exactly one convention
- Contain multiple sections
- Can be created by Owner and Administrator
- Can be edited by Owner and Administrator
- Can be deleted by Owner and Administrator only
...
- Optional fields: elder_friendly, handicap_friendly, hearing_loop, information
...
- Administrator: Can create, update, and delete all sections
- URL session users: Can update occupancy and report attendance only (no create/delete)

## Role-Based Access Control

### Two-Tier Authenticated Roles + URL Sessions

The system uses a two-tier authenticated role system supplemented by URL-based anonymous access, implemented via pivot tables (not a package like Spatie).

### Owner
- Full administrative privileges
- Can delete conventions
- Can export convention data
- Can regenerate URL tokens
- Has all Administrator capabilities
...
### Section URL Session (Anonymous)
- Access via shareable URL containing section_url_token
- Can view convention, floors, and sections
- Can update occupancy for any section
- Can report attendance for any section
- Cannot manage floors, sections, or users
- Cannot start/stop attendance reports
- Session stored with convention_id, type, and token for validation
- Token verification on each request (stale sessions cleared if token regenerated)

### Middleware Enforcement
- `EnsureConventionOrUrlAccess` — verifies user has a role OR valid URL session for the convention
- `EnsureOwnerRole` — restricts owner-only actions (delete, export)

## URL-Based Anonymous Access

### How It Works
1. Convention has an auto-generated `section_url_token` (64-char random string)
2. Owner/Administrator shares the URL: `/url-access/section/{token}`
3. Anonymous user visits the URL
4. `UrlAccessController` looks up convention by token, creates a session
5. Session stores: convention_id, type ('section'), token
6. `EnsureConventionOrUrlAccess` middleware validates the session on subsequent requests
7. If token is regenerated, stale sessions are automatically cleared

### Token Regeneration
- Authenticated users can regenerate the URL token via `POST /conventions/{convention}/regenerate-url-token`
- Regeneration invalidates all existing URL sessions for that convention
- New token is generated and old URLs stop working
...
4. If new user:
   ...
   b. Creates convention via CreateConventionAction (user becomes Owner + Administrator)
...
1. Convention manager creates user with email and roles (Owner and/or Administrator)
...
2. System removes all roles for that convention
...
### Starting a Report
1. Administrator/Owner clicks "START ATTENDANCE REPORT"
...
1. Users (authenticated or URL session) navigate to sections
2. Enter attendance count in numeric input
3. System creates/updates AttendanceReport record
4. Records reported_at timestamp
5. Counter updates: "X of Y sections reported"

### Stopping a Report
1. Administrator/Owner clicks "Stop attendance report"
...
### Update Restrictions
- Before lock: Any user with access can update attendance
- After lock: No updates allowed (period is immutable)
- Administrator/Owner override: Can lock period even if incomplete

## Search Functionality

### Available to All Users
- No role-based filtering applied
- Accessible to any user with convention access (authenticated or URL session)
...
- Hearing loop: Checkbox to filter sections with hearing_loop=true
...
- Section details (capacity, occupancy, accessibility including hearing_loop)
...
- Optional: floor_id (sometimes required, from route or request body), elder_friendly, handicap_friendly, hearing_loop, information
...
### Section Update
- Required: name, number_of_seats
- Optional: elder_friendly, handicap_friendly, hearing_loop, information
...
- Available roles: Owner, Administrator
...
- Same role conditional requirements
...
- URL session validation with token verification
